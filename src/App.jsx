...
/* ------------------------------------------------------------------ */
/* Status helpers — one source of truth for how a status renders       */
/* ------------------------------------------------------------------ */
const STATUS_META = {
  Pending: { label: 'Pending', badge: 'bg-[#253745] text-[#9BA8AB]' },
  Claimed: { label: 'Claimed', badge: 'bg-[#FF9800]/20 text-[#FF9800]' },
  'Under Review': { label: 'Under AI Review', badge: 'bg-[#4A90D9]/20 text-[#4A90D9]' },
  Completed: { label: 'Completed', badge: 'bg-[#6BBF4A]/20 text-[#6BBF4A]' },
  Flagged: { label: 'Flagged', badge: 'bg-[#F44336]/20 text-[#F44336]' }
};
 ...
 * using Google's Gemini 1.5 Flash vision model — chosen because Google AI
 * Studio issues free API keys with no credit card required (sign in with a
 * Google account at https://aistudio.google.com/app/apikey), unlike OpenAI
 * or Anthropic which gate their APIs behind billing. This is the ONLY place
 * a verdict is produced — nothing upstream is allowed to pre-decide
 * approval. The model reasons about four independent factors and we derive
 * the final "approved" call ourselves from those factors rather than
 * trusting a single self-reported boolean.
 *
 ...
 * visible to anyone who opens devtools. That's fine for a free-tier
 * prototype, but before shipping this to real users, move this fetch into a
 * Supabase Edge Function (or any small backend) that holds the key
 * server-side and proxies the request — never ship a production API key in
 * frontend JS.
 *
...
async function runPramaanAudit(issue, proof, onStep) {
  ...
  if (!apiKey) {
    throw new Error("Missing VITE_GEMINI_API_KEY in your .env file. Get a free key (no credit card) at https://aistudio.google.com/app/apikey and add it to enable live AI audits.");
  }
  ...
  const [beforeBase64, afterBase64] = await Promise.all([
    ...
    fetchImageAsBase64(proof.afterImg)
  ]);

  if (!beforeBase64 || !afterBase64) {
    throw new Error("Could not load the before/after photos for analysis. Please re-upload clear images and try again.");
  }

  onStep?.(3);

  const prompt = `You are Pramaan, an independent and skeptical civic-infrastructure auditor for a municipal transparency platform. Citizens report defects, officers claim to fix them, and you are the only check between a false claim and public funds being credited.

CASE FILE
- Category: ${issue.category}
- Citizen-reported issue: ${issue.description || issue.title}
- Reported location: ${issue.location}
- Officer's work log: ${proof.description}
- Officer-claimed repair cost: Rs. ${proof.amountSpent}

You are given two photographs. IMAGE 1 is the "before" photo taken when the defect was reported. IMAGE 2 is the "after" photo the officer submitted as proof of repair.

Think it through carefully, then judge:
1. issueResolved — does IMAGE 2 clearly show this specific defect fixed (not just a different angle, an unrelated patch, or an unchanged scene)?
2. locationConsistent — do the surroundings, structures and landmarks in both photos plausibly show the same physical spot?
3. costReasonable — is the claimed cost plausible for this category of civic repair in an Indian municipal context? Give your own independent benchmark estimate in INR, based on typical PWD/municipal rates.
4. tamperingDetected — any visual sign of digital editing, cloning, mismatched lighting or shadows, resolution inconsistencies, or a reused/stock photo standing in for the after shot?

Weigh all four together. Only mark approved true if the defect is genuinely resolved, the location is consistent, and there is no tampering.

Respond with ONLY a raw JSON object — no markdown fences, no prose before or after — in exactly this shape:
{
  "issueResolved": boolean,
  "issueResolvedNote": "one short sentence",
  "locationConsistent": boolean,
  "locationNote": "one short sentence",
  "costReasonable": boolean,
  "benchmarkCostEstimate": number,
  "costNote": "one short sentence",
  "tamperingDetected": boolean,
  "tamperingNote": "3 to 6 words",
  ...
  try {
    ...
    throw new Error("Pramaan could not parse the AI verdict. Please retry the audit.");
  }

  const issueResolved = Boolean(parsed.issueResolved);
  const locationConsistent = Boolean(parsed.locationConsistent);
  const tamperingDetected = Boolean(parsed.tamperingDetected);
  const confidence = Math.max(1, Math.min(100, Number(parsed.confidence) || 0));

  // Safety net: derive the final call from the sub-checks ourselves rather
  // than blindly trusting a single "approved" flag. Tampering, an unresolved
  // defect, or a mismatched location always block approval no matter what
  // the model's own summary boolean says.
  const finalApproved =
    Boolean(parsed.approved) && issueResolved && locationConsistent && !tamperingDetected && confidence >= 55;

  return {
    issueResolved,
    issueResolvedNote: parsed.issueResolvedNote || '',
    locationConsistent,
    locationNote: parsed.locationNote || '',
    costReasonable: Boolean(parsed.costReasonable),
    benchmarkCostEstimate: Number(parsed.benchmarkCostEstimate) || 0,
    costNote: parsed.costNote || '',
    tamperingDetected,
    tamperingNote: parsed.tamperingNote || (tamperingDetected ? 'Possible tampering' : 'None detected'),
    confidence,
    approved: finalApproved,
    summary: parsed.summary || '',
    reportedCost: proof.amountSpent,
    beforeImg: proof.beforeImg,
    afterImg: proof.afterImg,
    auditedAt: new Date().toISOString(),
    coinsAwarded: finalApproved ? Math.round(200 + confidence * 3) : 0
  };
}
...
export default function App() {
  ...
  // Officer has submitted before/after photos, receipt and cost. We do NOT
  // decide pass/fail here — that verdict belongs to the AI audit alone, run
  // on the Pramaan screen.
  const handleSabootSubmit = (issue, proofData) => {
    const underReview = {
      ...issue,
      status: 'Under Review',
      proofData,
    ...
    setIssues(prev => prev.map(i => i.id === underReview.id ? underReview : i));
    handleOpenPramaan(underReview);
  };

  // The single place an AI verdict becomes a real status change.
  const handleVerificationComplete = (issueId, verdict) => {
    const status = verdict.approved ? 'Completed' : 'Flagged';
    setIssues(prev => prev.map(issue =>
      issue.id === issueId ? { ...issue, status, verificationData: verdict } : issue
    ));
    setActivePramaanIssue(prev =>
      prev && prev.id === issueId ? { ...prev, status, verificationData: verdict } : prev
    );
  };
            ...
            <main className="flex-1 p-8 max-w-[1440px] w-full mx-auto">
              ...
              {currentScreen === 'taskboard' && (
                <ScreenOfficerTaskBoard
                  ...
                  onOpenPramaan={handleOpenPramaan}
                />
              )}
              ...
              {currentScreen === 'pramaan' && (
                <ScreenAIVerification
                  ...
                  onResubmit={(issue) => handleOpenSaboot(issue)}
                />
              )}
              ...
              <div className="relative h-72 w-full bg-[#06141B] overflow-hidden">
                ...
                <div className="absolute top-4 left-4 flex gap-2">
                  ...
                  {issue.status && issue.status !== 'Pending' && (
                    <span className={`px-3 py-1 backdrop-blur-md text-[12px] uppercase font-semibold rounded-md flex items-center gap-1 ${getStatusMeta(issue.status).badge}`}>
                      {issue.status === 'Under Review' && <Icon name="RefreshCw" size={12} className="animate-spin" />}
...
function ScreenOfficerTaskBoard({ issues, user, onClaim, onOpenSaboot, onOpenPramaan }) {
  ...
  const queueIssues = issues.filter(i => i.status === 'Pending');
  const workspaceIssues = issues.filter(i => ['Claimed', 'Under Review', 'Completed', 'Flagged'].includes(i.status));

  const handleWorkspaceClick = (issue) => {
  ...
  const workspaceCta = (status) => {
    switch (status) {
      case 'Claimed': return 'Submit Saboot Proof →';
      case 'Under Review': return 'View Live Audit →';
      case 'Completed': return 'View Audit Certificate →';
      case 'Flagged': return 'View Flag Report →';
      default: return 'View Details →';
    }
  };

  const workspaceIcon = (status) => {
    switch (status) {
    ...
    <div className="w-full">
      <div className="flex items-center justify-between mb-8 pb-4 border-b border-[#253745]">
        ...
        <div className="flex bg-[#11212D] border border-[#9BA8AB]/20 rounded-lg p-1">
          ...
          >
            Issues Queue ({queueIssues.length})
          </button>
          <button
            onClick={() => setTab('workspace')}
            className={`px-5 py-2 text-[14px] uppercase font-semibold rounded-md btn-interact ${
              tab === 'workspace' ? 'bg-[#4A5C6A] text-[#CCD0CF] shadow' : 'text-[#9BA8AB] hover:text-[#CCD0CF]'
            }`}
          >
            My Workspace ({workspaceIssues.length})
          </button>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-6">
          {workspaceIssues.length === 0 ? (
            ...
            workspaceIssues.map((issue) => {
              ...
              return (
                <div key={issue.id} onClick={() => handleWorkspaceClick(issue)} className="bg-[#11212D] border border-[#9BA8AB]/20 rounded-xl overflow-hidden card-shadow p-5 hover:border-[#6BBF4A]/50 cursor-pointer btn-interact">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <span className="px-2.5 py-0.5 rounded text-[11px] uppercase font-semibold bg-[#253745] text-[#9BA8AB]">
                        {issue.category}
                      </span>
                      <span className={`px-2.5 py-0.5 rounded text-[11px] uppercase font-semibold flex items-center gap-1 ${meta.badge}`}>
                        <Icon name={workspaceIcon(issue.status)} size={12} className={issue.status === 'Under Review' ? 'animate-spin' : ''} />
                        {meta.label}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-[#253745]">
                    <span className="text-[13px] text-[#9BA8AB]">
                      Status: <strong className="text-[#CCD0CF]">{meta.label}</strong>
                    </span>
                    <span className="text-[13px] uppercase font-semibold text-[#6BBF4A]">
                      {workspaceCta(issue.status)}
                    </span>
                  </div>
                </div>
              );
...
function ScreenSabootSubmission({ issue, onBack, onSubmit }) {
  const [beforeImage, setBeforeImage] = useState(issue.proofData?.beforeImg || issue.image || null);
  const [afterImage, setAfterImage] = useState(issue.proofData?.afterImg || null);
  const [receiptFile, setReceiptFile] = useState(issue.proofData?.receiptFileName || null);
  const [amountSpent, setAmountSpent] = useState(issue.proofData?.amountSpent ? String(issue.proofData.amountSpent) : '');
  const [description, setDescription] = useState(issue.proofData?.description || '');

  const autoGeo = issue.location || "12.9716° N, 77.5946° E (Ward 112)";
  const autoTimestamp = new Date().toLocaleString('en-IN', { dateStyle: 'medium', timeStyle: 'medium' }) + ' IST (GPS Sync)';
  ...
  return (
    <div className="max-w-4xl mx-auto w-full">
      ...
      {issue.status === 'Flagged' && issue.verificationData && (
        <div className="mb-6 p-4 rounded-lg bg-[#F44336]/10 border border-[#F44336]/40 text-[13px] text-[#F44336]">
          <strong className="block mb-1 uppercase text-[11px] tracking-wider">Previous Audit Flagged This Submission</strong>
          {issue.verificationData.summary || 'The AI auditor could not verify this repair. Please review and resubmit clearer proof.'}
        </div>
      )}
...
/* ------------------------------------------------------------------ */
/* Pramaan AI Verification                                               */
/* ------------------------------------------------------------------ */
function ScreenAIVerification({ issue, userRole, onBack, onVerificationComplete, onResubmit }) {
  const storedVerdict = issue.verificationData || null;
  const proof = issue.proofData || null;
  const alreadyAudited = Boolean(storedVerdict);
  const canAudit = Boolean(proof) && issue.status === 'Under Review' && !alreadyAudited;

  const [step, setStep] = useState(alreadyAudited ? 6 : 0);
  const [auditState, setAuditState] = useState(alreadyAudited ? 'done' : (canAudit ? 'idle' : 'unavailable'));
  const [liveVerdict, setLiveVerdict] = useState(null);
  const [error, setError] = useState(null);
  const startedRef = useRef(false);

  const verdict = storedVerdict || liveVerdict;

  // Reset local audit state whenever we switch to a different issue.
  useEffect(() => {
    startedRef.current = false;
    setStep(alreadyAudited ? 6 : 0);
    setAuditState(alreadyAudited ? 'done' : (canAudit ? 'idle' : 'unavailable'));
    setLiveVerdict(null);
    setError(null);
  ...
  useEffect(() => {
    if (!canAudit || startedRef.current) return;
  ...
  async function runAudit() {
    ...
    setStep(1);
    try {
      const result = await runPramaanAudit(issue, proof, (s) => setStep(s));
      setStep(6);
      setLiveVerdict(result);
      setAuditState('done');
      onVerificationComplete(issue.id, result);
    } catch (err) {
      console.error("Pramaan audit failed:", err);
      setError(err.message || "The AI audit could not be completed.");
      setAuditState('error');
    }
  }
  ...
  const isAnalyzing = auditState === 'running';
  ...
  return (
    <div className="max-w-4xl mx-auto w-full">
            ...
            <div className="flex items-center gap-2">
              ...
              {auditState !== 'unavailable' && (
                <span className={`px-2 py-0.5 rounded text-[11px] font-bold uppercase tracking-wider ${
                  isAnalyzing
                    ? 'bg-[#FF9800]/20 text-[#FF9800] animate-pulse'
                    : auditState === 'error'
                    ? 'bg-[#F44336]/20 text-[#F44336]'
                    : 'bg-[#6BBF4A]/20 text-[#6BBF4A]'
                }`}>
                  {isAnalyzing ? 'Processing Live Audit...' : auditState === 'error' ? 'Audit Failed' : alreadyAudited ? 'Stored Verdict' : 'Autonomous Complete'}
                </span>
              )}
            </div>
            <p className="text-[13px] text-[#9BA8AB]">
              Case {issue.id} — {issue.title}. Multi-modal audit inspecting location consistency, resolution proof, cost variance & tamper detection.
            </p>
          </div>
        </div>
      </div>
      ...
      {auditState === 'unavailable' && (
        <div className="p-10 bg-[#11212D] border border-[#9BA8AB]/15 rounded-xl text-center">
          <Icon name="ShieldCheck" size={32} className="mx-auto text-[#9BA8AB] mb-3" />
          <h3 className="font-bebas text-[22px] text-[#CCD0CF] mb-1">No Saboot Proof Submitted Yet</h3>
          <p className="text-[14px] text-[#9BA8AB] max-w-md mx-auto">
            This issue hasn't reached the audit stage. An officer needs to claim it and submit before/after proof before Pramaan can run a verification.
          </p>
        </div>
      )}

      {error && (
        <div className="p-6 bg-[#F44336]/10 border border-[#F44336] rounded-xl text-[#F44336] mb-4">
          <h3 className="font-bebas text-[24px]">AI Audit Failed</h3>
          <p className="text-[14px] mt-2">{error}</p>
          <button onClick={handleRetry} className="mt-4 px-4 py-2 bg-[#F44336]/20 hover:bg-[#F44336]/30 rounded-lg text-[13px] uppercase font-semibold btn-interact flex items-center gap-2">
            <Icon name="RefreshCw" size={16} /> Retry Audit
          </button>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          <div className={`p-5 rounded-xl border transition-all duration-500 flex items-center justify-between ${step >= 1 ? 'bg-[#11212D] border-[#9BA8AB]/20 opacity-100 translate-y-0 card-shadow' : 'bg-[#11212D]/20 border-transparent opacity-0 translate-y-4'}`}>
            <div className="flex items-center gap-4">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                step < 6 ? 'bg-[#FF9800]/20 text-[#FF9800] animate-pulse' : verdict?.locationConsistent ? 'bg-[#6BBF4A]/20 text-[#6BBF4A]' : 'bg-[#F44336]/20 text-[#F44336]'
              }`}>
                <Icon name={step < 6 ? "RefreshCw" : (verdict?.locationConsistent ? "CheckCircle2" : "XCircle")} size={22} />
              </div>
              <div>
                <h4 className="font-bebas text-[18px] text-[#CCD0CF]">1. Location & Surroundings Consistency</h4>
                <p className="text-[12px] text-[#9BA8AB] max-w-md">
                  {step >= 6 && verdict ? (verdict.locationNote || 'Assessed against the before photo.') : 'Comparing before/after surroundings against the reported location...'}
                </p>
              </div>
            </div>
            {step >= 6 && verdict && (
              <span className={`font-bebas text-[18px] shrink-0 ${verdict.locationConsistent ? 'text-[#6BBF4A]' : 'text-[#F44336]'}`}>
                {verdict.locationConsistent ? '✓ Consistent' : '✕ Mismatch'}
              </span>
            )}
          </div>

          {/* Step 2: Issue actually resolved */}
          <div className={`p-5 rounded-xl border transition-all duration-500 flex items-center justify-between ${step >= 3 ? 'bg-[#11212D] border-[#9BA8AB]/20 opacity-100 translate-y-0 card-shadow' : 'bg-[#11212D]/20 border-transparent opacity-0 translate-y-4'}`}>
            <div className="flex items-center gap-4">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                step < 6 ? 'bg-[#FF9800]/20 text-[#FF9800] animate-pulse' : verdict?.issueResolved ? 'bg-[#6BBF4A]/20 text-[#6BBF4A]' : 'bg-[#F44336]/20 text-[#F44336]'
              }`}>
                <Icon name={step < 6 ? "RefreshCw" : (verdict?.issueResolved ? "CheckCircle2" : "XCircle")} size={22} />
              </div>
              <div>
                <h4 className="font-bebas text-[18px] text-[#CCD0CF]">2. Visual Proof of Resolution</h4>
                <p className="text-[12px] text-[#9BA8AB] max-w-md">
                  {step >= 6 && verdict ? (verdict.issueResolvedNote || 'Assessed from the after photo.') : 'Checking whether the after photo shows the reported defect actually fixed...'}
                </p>
              </div>
            </div>
            {step >= 6 && verdict && (
              <span className={`font-bebas text-[18px] shrink-0 ${verdict.issueResolved ? 'text-[#6BBF4A]' : 'text-[#F44336]'}`}>
                {verdict.issueResolved ? '✓ Resolved' : '✕ Not Evident'}
              </span>
            )}
          </div>
          ...
          <div className={`p-5 rounded-xl border transition-all duration-500 ${step >= 3 ? 'bg-[#11212D] border-[#9BA8AB]/20 opacity-100 translate-y-0 card-shadow' : 'bg-[#11212D]/20 border-transparent opacity-0 translate-y-4'}`}>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                  step < 6 ? 'bg-[#FF9800]/20 text-[#FF9800] animate-pulse' : verdict?.costReasonable ? 'bg-[#6BBF4A]/20 text-[#6BBF4A]' : 'bg-[#F44336]/20 text-[#F44336]'
                }`}>
                  <Icon name={step < 6 ? "RefreshCw" : (verdict?.costReasonable ? "CheckCircle2" : "XCircle")} size={22} />
                </div>
                <div>
                  <h4 className="font-bebas text-[18px] text-[#CCD0CF]">3. Cost Reasonableness & Audit Benchmark</h4>
                  <p className="text-[12px] text-[#9BA8AB]">
                    {step >= 6 && verdict ? (verdict.costNote || 'Compared against an independent AI benchmark.') : 'Benchmarking claimed cost against typical repair rates for this category...'}
                  </p>
                </div>
              </div>
            </div>
            <div className="bg-[#06141B] p-4 rounded-lg border border-[#9BA8AB]/15 space-y-3">
              <div>
                <div className="flex justify-between text-[12px] mb-1 font-semibold">
                  ...
                  <span className="text-[#6BBF4A]">₹{proof?.amountSpent ?? verdict?.reportedCost ?? '—'}</span>
                </div>
                <div className="h-4 bg-[#253745] rounded-full overflow-hidden">
                  <div
                    className="h-full bg-[#6BBF4A] rounded-full transition-all duration-700"
                    style={{ width: `${Math.min(100, ((proof?.amountSpent || 0) / Math.max(verdict?.benchmarkCostEstimate || 8000, proof?.amountSpent || 0, 1)) * 100)}%` }}
                  />
                </div>
              </div>
              {step >= 6 && verdict && (
                <div>
                  <div className="flex justify-between text-[12px] mb-1 font-semibold">
                    <span className="text-[#CCD0CF]">Pramaan Benchmark Estimate</span>
                    <span className="text-[#9BA8AB]">₹{verdict.benchmarkCostEstimate || '—'}</span>
                  </div>
                  <div className="h-4 bg-[#253745] rounded-full overflow-hidden">
                    <div
                      className="h-full bg-[#4A5C6A] rounded-full transition-all duration-700"
                      style={{ width: `${Math.min(100, ((verdict.benchmarkCostEstimate || 0) / Math.max(verdict.benchmarkCostEstimate || 0, proof?.amountSpent || 0, 1)) * 100)}%` }}
                    />
                  </div>
                </div>
              )}
          ...
          <div className={`p-5 rounded-xl border transition-all duration-500 flex items-center justify-between ${step >= 5 ? 'bg-[#11212D] border-[#9BA8AB]/20 opacity-100 translate-y-0 card-shadow' : 'bg-[#11212D]/20 border-transparent opacity-0 translate-y-4'}`}>
            <div className="flex items-center gap-4">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center ${step >= 6 ? (!verdict?.tamperingDetected ? 'bg-[#6BBF4A]/20 text-[#6BBF4A]' : 'bg-[#F44336]/20 text-[#F44336]') : 'bg-[#FF9800]/20 text-[#FF9800] animate-spin'}`}>
                <Icon name={step >= 6 ? (!verdict?.tamperingDetected ? "CheckCircle2" : "XCircle") : "RefreshCw"} size={22} />
              </div>
              <div>
                <h4 className="font-bebas text-[18px] text-[#CCD0CF]">4. Neural Image Tampering Check</h4>
                <p className="text-[12px] text-[#9BA8AB]">Analyzing lighting, artifacts, and before/after topological match.</p>
              </div>
            </div>
            <span className={`font-bebas text-[18px] shrink-0 ${step >= 6 ? (!verdict?.tamperingDetected ? 'text-[#6BBF4A]' : 'text-[#F44336]') : 'text-[#FF9800]'}`}>
              {step >= 6 && verdict ? `Verdict: ${verdict.tamperingNote}` : 'Analyzing Photos...'}
            </span>
          </div>

          {/* Final confidence + verdict banner */}
          <div className={`p-6 rounded-xl border transition-all duration-700 ${step >= 6 ? 'bg-[#11212D] border-[#9BA8AB]/30 opacity-100 translate-y-0 card-shadow' : 'bg-[#11212D]/20 border-transparent opacity-0 translate-y-4'}`}>
            <div className="flex items-center justify-between gap-6">
              <div>
                <h4 className="font-bebas text-[24px] text-[#CCD0CF] mb-1">5. Live Pramaan Confidence Score</h4>
                <p className="text-[13px] text-[#9BA8AB] max-w-md">
                  {verdict?.summary || 'Synthesized from location, resolution, cost and tampering checks combined.'}
                </p>
              </div>
              ...
              <div className="relative w-28 h-28 flex items-center justify-center shrink-0">
                <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                  ...
                  <circle
                    cx="50" cy="50" r="40" stroke={(verdict?.confidence || 0) > 75 ? "#6BBF4A" : "#F44336"} strokeWidth="8"
                    strokeDasharray="251.2" strokeDashoffset={251.2 - (251.2 * (verdict?.confidence || 0)) / 100}
                    strokeLinecap="round" fill="transparent" className="transition-all duration-1000 ease-out"
                  />
                </svg>
                <div className="absolute flex flex-col items-center justify-center">
                  <span className="font-bebas text-[28px] text-[#CCD0CF] leading-none">{verdict?.confidence || 0}%</span>
              ...
              <div className="mt-6 pt-6 border-t border-[#253745]">
                {verdict.approved ? (
                  <div className="p-4 rounded-xl bg-[#6BBF4A]/15 border border-[#6BBF4A] flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-[#6BBF4A] text-black flex items-center justify-center font-bold shrink-0">✓</div>
                      <div>
                        ...
                        <p className="text-[12px] text-[#CCD0CF]">
                          Verified and logged.{verdict.coinsAwarded ? ` ${verdict.coinsAwarded} Seva Coins awarded.` : ''}
                        </p>
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="p-4 rounded-xl bg-[#F44336]/15 border border-[#F44336] flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-[#F44336] text-white flex items-center justify-center font-bold shrink-0">!</div>
                      <div>
                        <h3 className="font-bebas text-[22px] text-[#F44336] leading-tight m-0">Flagged for Review</h3>
                        <p className="text-[12px] text-[#CCD0CF]">
                          {verdict.tamperingDetected
                            ? 'Possible tampering detected in the submitted photos.'
                            : !verdict.issueResolved
                            ? 'The after photo does not clearly show the defect resolved.'
                            : !verdict.locationConsistent
                            ? 'The before/after locations do not appear consistent.'
                            : 'The AI could not confirm this repair with enough confidence.'}
                        </p>
                      </div>
                    </div>
                    {userRole === 'officer' && (
                      <button
                        onClick={() => onResubmit(issue)}
                        className="px-4 py-2 bg-[#F44336] text-white text-[12px] uppercase font-semibold rounded-lg btn-interact shrink-0"
                      >
                        Resubmit Proof
                      </button>
                    )}
              ...
              {issues.map((item) => (
                ...
                >
                  ...
                  <td className="py-3.5 px-4">
                    <span className={`px-2 py-0.5 rounded text-[11px] uppercase font-bold ${getStatusMeta(item.status).badge}`}>
