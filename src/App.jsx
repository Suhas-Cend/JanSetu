  ...
  useEffect(() => {
    async function runAIAudit() {
      try {
        ...
        if (!apiKey) {
          throw new Error("Missing VITE_GEMINI_API_KEY in your .env file. Please add your AQ... key.");
        }
        ...
        const prompt = `
          ...
          Respond ONLY with a valid JSON object matching this exact structure (no markdown tags, no extra text):
          {
            ...
            "confidence": <number between 1 and 100 representing your intelligent score based on visual proof>,
            "approved": <boolean>
          }
        `;
          ...
          <div className={`p-6 rounded-xl border transition-all duration-700 ${step >= 6 ? 'bg-[#11212D] border-[#9BA8AB]/30 opacity-100 translate-y-0 card-shadow' : 'bg-[#11212D]/20 border-transparent opacity-0 translate-y-4'}`}>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-[13px] text-[#9BA8AB] max-w-md">Synthesized dynamically by Google Gemini based on multimodal visual analysis.</p>
              </div>
